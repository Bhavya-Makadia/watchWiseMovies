...
func GetRecommendedMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadGateway, gin.H{"error": "User Id not found in context"})
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error fetching reccomended movies"})
			return
		}
...
func GetUsersFavouriteGenres(userId string, client *mongo.Client, c *gin.Context) ([]string, error) {
	...
	if !ok {
		return []string{}, errors.New("unable to retrieve fav genre for user")
	}
...
func GetGenres(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error fetching movie genre"})
			return
		}
		defer cursor.Close(ctx)
